...
def position_data(trial: int, joint: str):
    ...
    represent the position of the joint, as time passes within the
    selected trial.
    """
...
def total_joint_data(joint: str):
    ...
    for i in range(1, 126):
        ...
        trial_data.columns = [
            f"trial{i}_{col}" if col != "time" else "time" for col in trial_data.columns
        ]
    ...
    averaged_data = pd.DataFrame(
        {"time": time, "avg_x": avg_x, "avg_y": avg_y, "avg_z": avg_z}
    )
...
def append_jerk(results):
    ...
    acceleration_x = np.gradient(
        np.gradient(results["x_lowess"], results["time"]), results["time"]
    )
    acceleration_y = np.gradient(
        np.gradient(results["y_lowess"], results["time"]), results["time"]
    )
    acceleration_z = np.gradient(
        np.gradient(results["z_lowess"], results["time"]), results["time"]
    )
    ...
    for jerk_column in jerk_columns:
        ...
        for extrema_index in extrema:
            ...
            if (
                ...
                and extrema_index not in main_extrema
                and not has_edge_case
            ):
...
def segment(joint: str):
    ...
    plt.plot(
        results["time"],
        results["x_lowess"],
        label="Normalized X LOWESS",
        color="orange",
    )
    plt.plot(
        results["time"], results["jerk_x"], label="Normalized Jerk X", color="green"
    )
    plt.scatter(
        results["time"].iloc[x],
        results["jerk_x"].iloc[x],
        color="red",
        label="Significant X Extrema",
        zorder=5,
    )
    ...
    plt.plot(
        results["time"],
        results["y_lowess"],
        label="Normalized Y LOWESS",
        color="orange",
    )
    plt.plot(
        results["time"], results["jerk_y"], label="Normalized Jerk Y", color="green"
    )
    plt.scatter(
        results["time"].iloc[y],
        results["jerk_y"].iloc[y],
        color="red",
        label="Significant Y Extrema",
        zorder=5,
    )
    ...
    plt.plot(
        results["time"],
        results["z_lowess"],
        label="Normalized Z LOWESS",
        color="orange",
    )
    plt.plot(
        results["time"], results["jerk_z"], label="Normalized Jerk Z", color="green"
    )
    plt.scatter(
        results["time"].iloc[z],
        results["jerk_z"].iloc[z],
        color="red",
        label="Significant Z Extrema",
        zorder=5,
    )
    ...
    plt.show()
    return (
        results["time"].iloc[x].to_numpy(),
        results["time"].iloc[y].to_numpy(),
        results["time"].iloc[z].to_numpy(),
    )
