...
def extract_joint_positions(json_data):
    ...
    for frame in json_data['tracking']:
        ...
        # TODO: Include an option to include this as a variable
        r_elbow = np.array(player_data['R_ELBOW'])
        r_wrist = np.array(player_data['R_WRIST'])

        positions.append((r_shoulder, r_elbow, r_wrist))
        # Should probably save this as a  np.array but this works for now
    return positions
...
def frame_to_rotation(positions):
    ...
    prev_upper_arm, prev_forearm = calculate_limb_vectors(positions[0])
    prev_upper_arm, prev_forearm = prev_upper_arm / \
        np.linalg.norm(prev_upper_arm), prev_forearm / \
        np.linalg.norm(prev_forearm)
    ...
    for i in range(1, len(positions)):
        ...
        upper_arm_kmat = np.array([[0, -upper_arm_cross_prod[2],
                                    upper_arm_cross_prod[1]],
                                   [upper_arm_cross_prod[2], 0,
                                    -upper_arm_cross_prod[0]],
                                   [-upper_arm_cross_prod[1],
                                    upper_arm_cross_prod[0], 0]])

        upper_arm_rotation_matrix = np.eye(3) + upper_arm_sin*upper_arm_kmat +\
            (1-upper_arm_cos)*upper_arm_kmat.dot(upper_arm_kmat)
        ...
        forearm_kmat = np.array([[0, -forearm_cross_prod[2],
                                  forearm_cross_prod[1]],
                                 [forearm_cross_prod[2], 0,
                                  -forearm_cross_prod[0]],
                                 [-forearm_cross_prod[1],
                                  forearm_cross_prod[0], 0]])

        forearm_rotation_matrix = np.eye(3) + forearm_sin*forearm_kmat + \
            (1-forearm_cos)*forearm_kmat.dot(forearm_kmat)
        rot_matrices.append((upper_arm_rotation_matrix,
                             forearm_rotation_matrix))
