...
class ShapletLearner(nn.Module):
    def __init__(
        ...
        assert (
            input_size % len_shapelet == 0
        ), "The input size may not be compatiable with the Timeseries size"
        ...
        if kmeans_centroids is not None:  ## TODO Include method directly
            ...
            self.shapelets = nn.ParameterList(
                [
                    nn.Parameter(
                        torch.tensor(centroid, dtype=torch.float32).unsqueeze(0)
                    )
            ...
            self.shapelets = nn.ParameterList(
                nn.Parameter(torch.randn(self.K)) for _ in range(num_shapelet)
            )
