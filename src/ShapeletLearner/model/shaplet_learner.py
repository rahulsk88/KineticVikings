...
class ShapletLearner(nn.Module):
    def __init__(self, num_shapelet, len_shapelet, input_size, num_classes, 
                 alpha_precision, kmeans_centroids, type_dist):
        super().__init__()

        assert input_size % len_shapelet == 0, "The input size may not be \
            compatiable with the Timeseries size"
        ...
        if kmeans_centroids is not None:  # TODO Include method directly
            assert kmeans_centroids.shape == (num_shapelet, len_shapelet), \
                "KMeans centroids must have the shape (num_shapelets,  \
                    len_shapelet)."
            self.shapelets = nn.ParameterList(
                [nn.Parameter(torch.tensor(centroid,
                                           dtype=torch.float32).unsqueeze(0))
                    for centroid in kmeans_centroids]
            )
        else:
            self.shapelets = nn.ParameterList(nn.Parameter(torch.randn(self.K))
                                              for _ in range(num_shapelet))
