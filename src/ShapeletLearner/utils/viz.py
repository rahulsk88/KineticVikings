    ...
    for i, shapelet in enumerate(shapelets):
        ...
        padded_shapelet = torch.cat(
            [
                ...
                torch.full(
                    (len(timeseries.squeeze()) - (start_idx + shapelet_length),),
                    float("nan"),
                ),  # Padding after the shapelet
            ]
        )
        axes[i].plot(
            timeseries.squeeze().cpu().numpy(), label=f"Time Series", color="b"
        )
        axes[i].plot(
            padded_shapelet.cpu().detach().numpy(),
            color="g",
            linewidth=2,
            label=f"Shapelet {i+1}",
        )
        axes[i].legend()
